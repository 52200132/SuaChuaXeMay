...
async def get_all_part_order_details(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
async def get_all_part_order_details_by_order(order_id: int, db: Session = Depends(get_db)):
...
async def get_part_order_detail_by_id(part_detail_ID: int, db: Session = Depends(get_db)):
...
@router.post(URLS['PART_ORDER_DETAIL']['CREATE_PART_ORDER_DETAIL'], response_model=PartOrderDetailResponse, status_code=status.HTTP_201_CREATED)
async def create_part_order_detail(part_detail: PartOrderDetailCreate, db: Session = Depends(get_db)):
    try:
        db_part_detail = await crud.create_part_order_detail(db=db, part_detail=part_detail)
        return PartOrderDetailResponse.from_orm(db_part_detail)
        ...
        await db.rollback()
        logger.error(f"Lỗi không xác định khi tạo chi tiết phụ tùng đơn hàng: {str(e)} | Dữ liệu: {part_detail.dict()}")
...
async def update_part_order_detail(part_detail_ID: int, part_detail: PartOrderDetailUpdate, db: Session = Depends(get_db)):
    db_part_detail = await crud.update_part_order_detail(db, part_detail_ID=part_detail_ID, part_detail=part_detail)
    if db_part_detail is None:
        raise HTTPException(status_code=404, detail="Không tìm thấy chi tiết đơn hàng phụ tùng")
