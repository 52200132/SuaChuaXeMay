...
async def get_all_service_type(db: AsyncSession) -> list[ServiceTypeResponse]:
    """Lấy tất cả ServiceType từ cơ sở dữ liệu."""
    try:
        query = select(ServiceType)
        result = await db.execute(query)
        service_types = result.scalars().all()
        service_types = [ServiceTypeResponse.from_orm(service_type) for service_type in service_types]
        logger.info(f"Fetched {len(service_types)} service types from the database.")
...
async def get_service_type_by_id(db: AsyncSession, service_type_id: int) -> list[ServiceTypeResponse]:
